import { M_obrassociales } from "../model/obrassociales.js";
...
const mostrarTodo = async (req,res) => {
    try {        
    const items = await M_obrassociales.findAll();
    console.log(items);
    res.json(items); 
    } catch (error){
...
const crearItem = async (req, res) => {
    ...
    const nuevoItem = await M_obrassociales.create( {
        obrasocial
    } );
...
const mostrarUno = async (req,res) => {
    ...
    try {
        ...
        const item = await M_obrassociales.findByPk(id);
        // DAMOS RES
        res.json(item);
...
const borrarItem = async (req, res) => {
    try {
        ...
        /// Agregar control de realacion de tablas
         const pacientesVinculados =  await M_pacientes.findAll({
            where: {
                idObrasocial : id,
            }
        });
        ...
        if (pacientesVinculados.length > 0) {
            return res.status(400).json({error: 'Esta item no puede borrarse por estar vinculado a otros elementos'})
        }
 
    // CON SEQUELIZE DESTROY BORRAMOS EL ITEM
    await M_obrassociales.destroy({
        where: {
